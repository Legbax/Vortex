...
class IDsFragment : Fragment() {
    ...
    private fun validateAndSaveImsi(imsi: String): Boolean {
        // IMSI: exactamente 15 dígitos numéricos
        if (!imsi.matches(Regex("\\d{15}"))) {
            Toast.makeText(requireContext(), "IMSI inválido: debe tener exactamente 15 dígitos numéricos", Toast.LENGTH_SHORT).show()
            return false
        }
        // MCC: primeros 3 dígitos, rango 200-999
        val mcc = imsi.take(3).toIntOrNull() ?: run {
            Toast.makeText(requireContext(), "MCC inválido", Toast.LENGTH_SHORT).show()
            return false
        }
        if (mcc < 200 || mcc > 999) {
            Toast.makeText(requireContext(), "MCC fuera de rango (200-999): $mcc", Toast.LENGTH_SHORT).show()
            return false
        }
        // MNC para EE.UU. (310-316): 3 dígitos, rango 001-999
        if (mcc in 310..316) {
            val mnc = imsi.substring(3, 6).toIntOrNull() ?: run {
                Toast.makeText(requireContext(), "MNC inválido para carrier USA", Toast.LENGTH_SHORT).show()
                return false
            }
            if (mnc < 1 || mnc > 999) {
                Toast.makeText(requireContext(), "MNC USA fuera de rango (001-999): $mnc", Toast.LENGTH_SHORT).show()
                return false
            }
        }
        // Guardar si todas las validaciones pasan
